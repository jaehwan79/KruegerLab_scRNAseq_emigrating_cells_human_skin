...
levels(IL17A.Tcells)
new.cluster.ids <- c( "IL17A_CD161_T_cell_Psoriasis", "IL17A_CD4_T_cell_Psoriasis" ,  "IL17A_CD8_T_cell_Psoriasis"  , "IL17A_Treg_Psoriasis"   )
...
levels(IL17F.Tcells)
new.cluster.ids <- c("IL17F_CD161_T_cell_Psoriasis", "IL17F_CD4_T_cell_Psoriasis"  , "IL17F_Treg_Psoriasis"   )
...
#IL17A+ IL17F+ T-cell subset
Tcells <- subset(Round2, idents = c( "CD161_T_cell_Psoriasis", "CD8_T_cell_Psoriasis"   , "CD4_T_cell_Psoriasis" ,"Treg_Psoriasis" ))
IL17AF.Tcells <- subset(Tcells, subset = IL17F > 1)
IL17AF.Tcells <- subset(IL17AF.Tcells, subset = IL17A > 1)
levels(IL17AF.Tcells)
new.cluster.ids <- c("IL17AF_CD161_T_cell_Psoriasis", "IL17AF_CD4_T_cell_Psoriasis" ,  "IL17AF_Treg_Psoriasis"   )
...
T17_cells <- merge(x= IL17A.Tcells, y = c(IL17F.Tcells ,IL17AF.Tcells), project = "T17_cells")
new.cluster.ids <- c("CD161_T_cell" , 
                     "CD4_T_cell" ,
                     "CD8_T_cell"  ,  
                     "Treg"    ,    
                     "CD161_T_cell" ,
                     "CD4_T_cell"  ,
                      "Treg"   ,      "CD161_T_cell" , "CD4_T_cell" ,   "Treg"     )
names(new.cluster.ids) <- levels(T17_cells)
T17_cells <- RenameIdents(T17_cells, new.cluster.ids)
levels(Idents(T17_cells))

Idents(object = T17_cells) <- factor(Idents(object = T17_cells), 
                                             levels = (c(   "CD161_T_cell", "CD8_T_cell","CD4_T_cell"   ,   "Treg"    )))
levels(Idents(T17_cells))
...
pdf("Heatmap_gene_expression_T17_cells_sAverage_expression13.pdf", width=6,height=10)
...
#####################################################################################################################
#Total_cell_count ####
list <- as.data.frame(table(T17_cells@active.ident))
list[is.na(list)] <- 0
...
#i=30
#string <- paste("list2 <- subset(T17_cells, subset =",marker[i],"> 1)")
#eval(parse( text= string ))
#marker[i]

for (i in 1:length(marker)){
  string <- paste("list2 <- subset(T17_cells, subset =`",marker[i],"`> 1)", sep="")
  eval(parse( text= string ))
...
list[is.na(list)] <- 0
#####################################################################
# Heatmap for cell number ####
list_number<- list

#list_number<- as.data.frame(t(list_number))
rownames(list_number) <- list_number[,1]
list_number<- list_number[,-c(1,2)]
...
#data_subset_norm <- t(apply(list_number, 1, cal_z_score))

pdf("Heatmap_gene_expression_T17_cell_number02.pdf", width=6,height=10)
p <-   pheatmap(list_number, display_numbers = T,cluster_cols = F, cluster_rows=F,  scale = "none"
                ,colorRampPalette(c('white','red'))(100), number_format = "%.0f",
                , main = "T17 cells: Number of cells" )

print(p)
dev.off()

#####################################################################
# Heatmap for proportion cluster ####
...
#data_subset_norm <- t(apply(list_proportion_cluster, 1, cal_z_score))


pdf("Heatmap_gene_expression_T17_cell_proportion_cluster02.pdf", width=6,height=10)
p <-   pheatmap(list_proportion_cluster, display_numbers = T,cluster_cols = F, cluster_rows=T
                ,colorRampPalette(c('white','red'))(100)
                , number_format = "%.1f", main = "T17 cells: Proportion of cells in Cluster (%)" )
